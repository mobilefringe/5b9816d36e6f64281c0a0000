						...
						<ul>
						    <li class="menu_item" v-for="item in menu_items" :id="item.id">
						        ...
						        <ul v-if="item.sub_menu">
						            <li v-for="sub_menu in item.sub_menu" class="dropdown_item" :key="sub_menu.name">
    							        ...
    							        <div v-else>
    							            <b-card no-body class="mb-1">
                                                ...
                                                <b-collapse v-model="item.show_sub_menu" :id="$t(item.name)" :visible="item.show_sub_menu" :accordion="$t(item.name)" role="tabpanel" class="accordion_body">
                                                    <b-card-body v-for="sub_menu in item.sub_menu" :key="sub_menu.name">
                                                        <p class="card-text">
...
<script>
    define(["Vue", "vuex", "bootstrap-vue", "json!site.json", 'vue!search-component'], function (Vue, Vuex, BootstrapVue, site, SearchComponent) {
        console.log("SiteSearchComponent", SiteSearchComponent);
